...
try:
    from cross_examination_framework.utils import load_yaml
    from cross_examination_framework.QAG import (
        generate_qa_from_text,
        cross_examine,
        calculate_scores,
        calculate_summary_reduction,
    )
...
# --- Configuration ---
DEFAULT_CONFIG_PATH = "cross_examination_framework/pipeline_params.yaml"
DEFAULT_PROMPT_CATALOGUE_PATH = "cross_examination_framework/prompts.yaml"
...
class CrossExamRequest(BaseModel):
    ...
    config_path: Optional[str] = Field(
        ...
        description="Path to the pipeline parameters YAML file.",
    )
    prompt_catalogue_path: Optional[str] = Field(
        default=DEFAULT_PROMPT_CATALOGUE_PATH,
...
def _get_llm_opts(step_params: Dict, prompt_catalogue: Dict) -> Dict:
    ...
    system_instruction = prompt_catalogue.get("system_instruction_set", {}).get(
        system_instruction_key
    )
    user_instruction = prompt_catalogue.get("user_instruction_set", {}).get(
        user_instruction_key
    )
...
async def perform_cross_examination(
    ...
    logger.debug(
        f"RID: {request_id} - Original Doc Snippet: {original_document[:100]}..."
    )  # Log snippet
    logger.debug(
        f"RID: {request_id} - Generated Doc Snippet: {generated_document[:100]}..."
    )  # Log snippet
    ...
    try:
        ...
        llm_api_qa_gen_doc = LLM_Caller(
            qa_doc_params["base_endpoint"],
            api_key=api_key,
            prompt_filepath=prompt_catalogue_path,
        )
        qa_sum_endpoint = qa_sum_params.get(
            "base_endpoint", qa_doc_params["base_endpoint"]
        )
        llm_api_qa_gen_sum = LLM_Caller(
            qa_sum_endpoint, api_key=api_key, prompt_filepath=prompt_catalogue_path
        )
        llm_api_cross_exam = LLM_Caller(
            cross_exam_params["base_endpoint"],
            api_key=api_key,
            prompt_filepath=prompt_catalogue_path,
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"RID: {request_id} - Server setup error: {e}"
        )
    ...
    try:
        ...
        if generated_qa_from_document is not None:
            ...
            try:
                ...
                logger.info(
                    f"RID: {request_id} - Using provided QA pairs, converted to dicts."
                )
            except AttributeError:  # Fallback for Pydantic v1 if needed
                qa_from_doc = [item.dict() for item in generated_qa_from_document]
                logger.info(
                    f"RID: {request_id} - Using provided QA pairs (Pydantic v1 fallback), converted to dicts."
                )
            except Exception as conversion_err:
                logger.error(
                    f"RID: {request_id} - Failed to convert provided QAPair models to dicts: {conversion_err}",
                    exc_info=True,
                )
                qa_from_doc = (
                    None  # Handle error, maybe proceed without QA or raise exception
                )
        else:
            ...
            qa_from_doc = await generate_qa_from_text(
                ...
                num_questions=qa_doc_params.get(
                    "num_questions_to_generate", 5
                ),  # Use .get w/ default
                prompt_catalogue_path=prompt_catalogue_path,
            )
        qa_from_doc_repr = (
            f"Count: {len(qa_from_doc)}, First: {qa_from_doc[0]}"
            if qa_from_doc
            else "None or Empty"
        )
        ...
        if qa_from_doc is not None and (
            not isinstance(qa_from_doc, list)
            or not all("question" in item and "answer" in item for item in qa_from_doc)
        ):
            logger.warning(
                f"RID: {request_id} - Unexpected format for qa_from_doc: {type(qa_from_doc)}. Check QAG.generate_qa_from_text output."
            )
    except Exception as e:
    ...
    try:
        ...
        qa_from_summary = await generate_qa_from_text(
            ...
            num_questions=qa_sum_params.get(
                "num_questions_to_generate", 5
            ),  # Use .get w/ default
            prompt_catalogue_path=prompt_catalogue_path,
        )
        qa_from_summary_repr = (
            f"Count: {len(qa_from_summary)}, First: {qa_from_summary[0]}"
            if qa_from_summary
            else "None or Empty"
        )
        logger.debug(
            f"RID: {request_id} - Step 2 Result (qa_from_summary): {qa_from_summary_repr}"
        )
        if qa_from_summary is not None and (
            not isinstance(qa_from_summary, list)
            or not all(
                "question" in item and "answer" in item for item in qa_from_summary
            )
        ):
            logger.warning(
                f"RID: {request_id} - Unexpected format for qa_from_summary: {type(qa_from_summary)}. Check QAG.generate_qa_from_text output."
            )
    except Exception as e:
    ...
    try:
        ...
        logger.info(
            f"RID: {request_id} -   Step 3a: Answering Document questions using Generated Document"
        )
        ...
        summary_answers_repr = (
            f"Count: {len(summary_answers_to_doc_qs)}, First: {summary_answers_to_doc_qs[0]}"
            if summary_answers_to_doc_qs
            else "None or Empty"
        )
        logger.debug(
            f"RID: {request_id} - Step 3a Result (summary_answers_to_doc_qs): {summary_answers_repr}"
        )

        # 3b: Ask Summary Questions to Original Document
        logger.info(
            f"RID: {request_id} -   Step 3b: Answering Generated Document questions using Original Document"
        )
        ...
        doc_answers_repr = (
            f"Count: {len(doc_answers_to_summary_qs)}, First: {doc_answers_to_summary_qs[0]}"
            if doc_answers_to_summary_qs
            else "None or Empty"
        )
        logger.debug(
            f"RID: {request_id} - Step 3b Result (doc_answers_to_summary_qs): {doc_answers_repr}"
        )
    ...
    if summary_answers_to_doc_qs is not None or doc_answers_to_summary_qs is not None:
        try:
            ...
            mock_row = {
                ...
                qa_doc_key: (
                    qa_from_doc if qa_from_doc else []
                ),  # Use empty list if None
                qa_sum_key: (
                    qa_from_summary if qa_from_summary else []
                ),  # Use empty list if None
            ...
            logger.debug(
                f"RID: {request_id} - Mock Row Summary for calculate_scores: {mock_row_summary}"
            )

            # calculate_scores should handle None inputs for answer lists gracefully
            calculated_scores = calculate_scores(
                mock_row, doc_answers_key, sum_answers_key
            )
            logger.debug(
                f"RID: {request_id} - Raw calculated_scores: {calculated_scores}"
            )
            ...
            if not isinstance(calculated_scores, dict):
                logger.warning(
                    f"RID: {request_id} - calculate_scores did not return a dict. Got: {type(calculated_scores)}. Scores will be empty."
                )
                calculated_scores = {}
            ...
            valid_scores = [
                s
                for s in [coverage, conform, consistency]
                if isinstance(s, (int, float)) and s >= 0
            ]  # Ensure valid scores (e.g., >=0)
            ...
            logger.error(
                f"RID: {request_id} - Missing key during score calculation (Step 4): {e}",
                exc_info=True,
            )
            # Keep default empty scores
        except Exception as e:
            logger.error(
                f"RID: {request_id} - Error during score calculation (Step 4): {e}",
                exc_info=True,
            )
            # Keep default empty scores
    else:
        logger.warning(
            f"RID: {request_id} - Skipping score calculation as cross-examination results are missing."
        )

    # --- Construct Final Details ---
    details = {
        "qa_from_doc_count": len(qa_from_doc) if isinstance(qa_from_doc, list) else 0,
        "qa_from_summary_count": (
            len(qa_from_summary) if isinstance(qa_from_summary, list) else 0
        ),
...
async def evaluate_documents(request: CrossExamRequest = Body(...)):
    """
    ...
    logger.info(
        f"RID: {request_id} - Received evaluation request. Config path: {request.config_path}"
    )
    try:
        ...
        final_scores = result.get(
            "scores", CrossExamScores()
        )  # Default to empty scores if key missing
        details = result.get("details")

        logger.info(
            f"RID: {request_id} - Evaluation successful. Overall Score: {getattr(final_scores, 'overall_score', 'N/A')}"
        )
        return CrossExamResponse(scores=final_scores, details=details)
    except HTTPException as e:
        # Log FastAPI/HTTP errors specifically
        logger.error(
            f"RID: {request_id} - HTTPException during processing: {e.detail}",
            exc_info=True,
        )
        ...
        logger.error(
            f"RID: {request_id} - Unexpected error in endpoint handler: {e}",
            exc_info=True,
        )
        # Return a generic server error response
        raise HTTPException(
            status_code=500,
            detail=f"RID: {request_id} - Internal Server Error: An unexpected error occurred.",
        )
...
if __name__ == "__main__":
    ...
    )  # Use string format for uvicorn.run
