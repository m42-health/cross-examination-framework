...
# %%
ds_path = "/data/cef/boxscore/boxscore_original"
...
judge_model_workers = [8, 4, 5, 6]  # worker-8, worker-4, worker-5, worker-6
...
num_questions_trg = 10  # Questions from game summary
...
# %%
class BoxscoreQAGenerator:
    def __init__(self, model_name, worker, port, prompt_path, num_questions_src, num_questions_trg):
        self.system_prompt = "You are a professional sports journalist specializing in NBA basketball coverage."
        self.headers = {"Content-Type": "application/json"}
        self.api_url = f"http://{worker}:{port}/v1/chat/completions"
        self.model_name = model_name
        with open(prompt_path) as f:
            prompts = yaml.safe_load(f)
        # Prompt for generating questions from boxscore (src)
        self.prompt_boxscore = prompts["user_instruction_set"]["qa_gen_boxscore_v0"]
        # Prompt for generating questions from game summary (trg)
        self.prompt_summary = prompts["user_instruction_set"]["qa_gen_summary_v0"]
    ...
    def generate_questions_from_src(self, example):
        """Generate questions from the source boxscore (src)"""
        rendered_prompt = self.prompt_boxscore.format(
            text=example["src"], 
    ...
    def generate_questions_from_trg(self, example):
        """Generate questions from the target game summary (trg)"""
        rendered_prompt = self.prompt_summary.format(
            text=example["trg"], 
...
# %%
output_path = f"/data/cef/boxscore/boxscore_questions_{judge_model.replace('/', '+++')}"
