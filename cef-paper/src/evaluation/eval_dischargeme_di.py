...
def prepare_dischargeme_di_dataset(
    ...
    """
    Prepare HuggingFace dataset from DischargeME DI predictions and references.
    
    Args:
        predictions_path (str): Path to predictions JSONL file containing llm_gen_di
    ...
    Returns:
        DatasetDict: Dataset containing src (full discharge summary), trg (predicted DI), 
                     reference (ground truth DI), and hadm_id
    """
    # Load predictions
    predictions = []
    ...
    for pred in predictions:
        ...
        pred_dict[hadm_id] = {
            'prediction': pred.get('llm_gen_di', ''),
    ...
    for idx, row in reference.iterrows():
        ...
        if hadm_id in pred_dict:
            entry = {
                ...
                'trg': pred_dict[hadm_id]['prediction'],  # Predicted DI
                'reference': row['discharge_instructions']  # Reference DI
            }
...
def eval_dischargeme_di(
    ...
    prompt_catalogue_path: str = "../cef_framework/prompts/discharge_instructions.yaml",
    ...
    results_base_path: str = "/data/cef/dischargeme/results_di_cef"
):
    """
    Evaluate DischargeME DI predictions using CEF and traditional metrics.
    
    Performs evaluation of generated Discharge Instructions by comparing predicted
    and reference discharge instructions using CEF (conformity, consistency, coverage) and 
    ...
    # Prepare dataset
    print("\nPreparing dataset...")
    ds = prepare_dischargeme_di_dataset(
    ...
    if eval_cef:
        ...
        for split in ds:
            try:
                # For DischargeME DI, we evaluate predicted DI (trg) against reference DI
    ...
    if eval_cef or eval_traditional:
        ...
        print("\n" + "="*80)
        print("DISCHARGE INSTRUCTIONS EVALUATION RESULTS")
        print("="*80)
...
if __name__ == "__main__":
    fire.Fire(eval_dischargeme_di)

# %%
# Example usage:
# python eval_dischargeme_di.py \
#   --predictions_path=/data/cef/dischargeme/dischargeme/DeepSeek-V3.1/predictions/DeepSeek-V3.1_raw_di_responses_tmp.jsonl \
