...
DEFAULT_PROMPT_CATALOGUE_PATH = "prompts/prompts.yaml"
...
class CrossExamRequest(BaseModel):
    ...
    config: Optional[str | Dict[str, Any]] = Field(default=DEFAULT_CONFIG_PATH, description="Path to the pipeline parameters YAML file or direct configuration dictionary.")
...
async def perform_cross_examination(
    ...
    try:
        if isinstance(config, str):
            pipeline_params = load_yaml(config)
        else:
            pipeline_params = config
        logger.info(f"RID: {request_id} - Pipeline params: {pipeline_params}")
    ...
    details = {
        ...
        "doc_answers_to_summary_qs": doc_answers_to_summary_qs if doc_answers_to_summary_qs else [],
        "summary_answers_to_doc_qs": summary_answers_to_doc_qs if summary_answers_to_doc_qs else [],
    }
...
async def evaluate_documents(
    ...
    logger.info(f"RID: {request_id} - Received evaluation request. Config path: {request.config}")
    try:
