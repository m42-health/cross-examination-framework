...
def default_llm_instructions(llm_opts, llm_task: str, prompt_filepath: str | Path = "prompts.yaml"):
    """
    ...
    if "system_instruction" not in llm_opts:
        llm_opts["system_instruction"] = prompt_catalogue["system_instruction_set"]["basic_v0"]
    ...
    if "user_instruction" not in llm_opts:
        if llm_task == "gen_summary":
            ...
            raise ValueError(f"{llm_task} is not a defined LLM task. Default prompts only defined for the following tasks: 'gen_summary', 'gen_qa', 'gen_answer'.")

        llm_opts["user_instruction"] = prompt_catalogue["user_instruction_set"][instruct_key]

def generate_summary(document: str, llm_opts: dict, messages=None, llm_api: LLM_Caller = None) -> str:
    """
    ...
    if messages is not None:
        ...
        llm_opts["user_instruction"] = prompt_catalogue["user_instruction_set"]["gen_summary_v0"].format(text=document)
        # llm_opts["user_instruction"] = llm_opts["user_instruction"].format(text=document)

    # Perform LLM call
    if llm_api is None:
        llm_api = LLM_Caller()
    summary = llm_api.chat_completion(**llm_opts, response_type="summary")

    return summary



async def generate_qa_from_text(text: str, llm_opts: dict, num_questions: int = 5, 
                          llm_api: LLM_Caller = None, screen_answers: bool = False, prompt_catalogue_path: str = "") -> List[dict]:
    """
    ...
    llm_opts["user_instruction"] = prompt_catalogue["user_instruction_set"]["qa_gen_document_v0"].format(text=text, num_questions=num_questions, document_task="content of the document")
    # print(llm_opts["user_instruction"])
    # LLM call to generate question-answer pairs from the text
    if llm_api is None:
        llm_api = LLM_Caller()
    qa_pairs = await llm_api.chat_completion(**llm_opts, response_type="qa_pairs")
    ...
    if screen_answers and qa_pairs is not None:
        update_answers = []
        prompt_catalogue = load_yaml(prompt_catalogue_path)   # TODO: just feed in prompt catalogue from the onset
        llm_opts["user_instruction"] = prompt_catalogue["user_instruction_set"]["gen_answers_v0"]  # TODO: this is hardcoded too deep in the code
        for qa in qa_pairs:
            # print(f"\n~~\n{qa}\n")
            answer = await answer_questions_from_text(text=text, question=qa["question"], llm_opts=llm_opts, llm_api=llm_api)
            ...
            if "YES" in answer:
                ...
                update_answers.append({"question": qa["question"], "answer": "IDK"})  # TODO: how should we deal with this scenario
            else:
                print("LLM answer is indeterminable")

        qa_pairs = update_answers  #TODO: make sure new qa_pairs same length as previously?
    return qa_pairs



async def answer_questions_from_text(text: str, question: str, llm_opts: dict, llm_api: LLM_Caller = None, prompt_catalogue_path: str = "") -> str:
    """
    ...
    # Add task specific parameters to the user instruction
    prompt_catalogue = load_yaml(prompt_catalogue_path)
    llm_opts["user_instruction"] = prompt_catalogue["user_instruction_set"]["gen_answers_v0"].format(text=text, question=question)
    ...
    if llm_api is None:
        llm_api = LLM_Caller()
    answer = await llm_api.chat_completion(**llm_opts, response_type="answer")
    # print(answer)
    return answer


async def cross_examine(text: str, gold_text: str, cross_question_set: List[dict], llm_opts: dict, 
                  llm_api: LLM_Caller, prompt_catalogue_path: str) -> List[dict]:
    """
    ...
    if cross_question_set is None:
        ...
        for qa_pair in cross_question_set:
            pred_answer = await answer_questions_from_text(text=text, question=qa_pair.get("question", None), 
                                                    llm_opts=llm_opts, llm_api=llm_api, prompt_catalogue_path=prompt_catalogue_path)
            gold_answer = await answer_questions_from_text(text=gold_text, question=qa_pair.get("question", None), 
                                                    llm_opts=llm_opts, llm_api=llm_api, prompt_catalogue_path=prompt_catalogue_path)
            predicted_answers.append({"predicted": pred_answer, "gold": gold_answer, "question": qa_pair["question"]})
...
def calculate_scores(samp, key1, key2) -> dict:
    ...
    questions_on_description = [q for q in questions_on_description if q["gold"] == "YES" or q["gold"] == "NO"] if questions_on_description is not None else []
    questions_on_summaries = [q for q in questions_on_summaries if q["gold"] == "YES" or q["gold"] == "NO"] if questions_on_summaries is not None else []
    
    coverage = conform = total = len(questions_on_summaries)
    for question in questions_on_summaries:
        if question["predicted"] == "IDK":
            coverage -= 1
        elif question["predicted"] != question["gold"]:
            conform -= 1
    if total != 0:
        coverage /= total
        conform /= total
    ...
    for question in questions_on_description:
        if question["predicted"] != question["gold"]:
            fact -= 1
    if total != 0:
        fact /= total
