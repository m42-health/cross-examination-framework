...
class LLM_Caller:
    ...
    async def chat_completion(self,
        ...
        def parse_response(response_data, response_type: str):
            if response_type == "qa_pairs":
            ...
            try:
                ...
                if response.choices and len(response.choices) > 0:
                    ...
                    if message and message.content:
                        ...
                        logger.warning(f"Attempt: {attempt}. LLM response choice message or content is empty. Role: {message.role if message else 'N/A'}. Full choice: {response.choices[0]}")
                        continue
                else:
                    logger.warning(f"Attempt: {attempt}. LLM response did not contain any choices. Full response object: {response}")
                    continue
                parsed_content = parse_response(content, response_type)
                if parsed_content:
                    return parsed_content
                else:
                    logger.warning(f"Attempt: {attempt}. LLM response did not pass validation. Full response object: {response}")
                    continue 
            except Exception as e: # Catch unexpected errors during the process
                logger.error(f"Attempt {attempt}. Unexpected error during chat completion: {e}")
                # Wrapping in ValueError might obscure original type, consider re-raising e
        
        if not parsed_content:
            raise ValueError(f"Failed to generate content from LLM model {target_model} after {attempts} attempts.")
