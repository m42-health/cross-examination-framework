...
ds_path = f"/data/cef/cnndm/cnndm_questions_{judge_model.replace('/', '+++')}"
...
# %%
class SummarizationAnswerGenerator:
...
# 1. Questions from src, answered using src (should be YES)
# 2. Questions from src, answered using trg
# 3. Questions from trg, answered using trg (should be YES)
# 4. Questions from trg, answered using src
...
for idx, model in enumerate(judge_models):
    ...
    answer_generator = SummarizationAnswerGenerator(
...
# %%
output_path = f"/data/cef/cnndm/cnndm_answers_{judge_model.replace('/', '+++')}"
