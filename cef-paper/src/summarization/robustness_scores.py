...
# - src_src: Questions from src (article), answered using src
# - src_trg: Questions from src (article), answered using trg (summary)
# - trg_trg: Questions from trg (summary), answered using trg
# - trg_src: Questions from trg (summary), answered using src (article)
...
# %%
ds_path = f"/data/cef/cnndm/cnndm_answers_{judge_model.replace('/', '+++')}"
...
for answer_type in answer_types:
    ...
    print(f"(Questions from {'article' if answer_type.startswith('src') else 'summary'}, "
          f"answered using {'article' if answer_type.endswith('src') else 'summary'})")
...
for answer_type in answer_types:
    ...
    for judge_model in judge_models:
        ...
        # Load all datasets for this answer type
        dss = {}
        for jm in judge_models:
            ds_path = f"/data/cef/cnndm/cnndm_answers_{jm.replace('/', '+++')}"
            try:
