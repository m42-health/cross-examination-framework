...
# %%
ds_path = "/data/cef/cnndm/cnndm_original"
...
num_questions_src = 10  # Questions from article
num_questions_trg = 5   # Questions from summary
...
# %%
class SummarizationQAGenerator:
    def __init__(self, model_name, worker, port, prompt_path, num_questions_src, num_questions_trg):
        self.system_prompt = "You are a helpful assistant specialized in text comprehension and summarization evaluation."
        self.headers = {"Content-Type": "application/json"}
        self.api_url = f"http://{worker}:{port}/v1/chat/completions"
        self.model_name = model_name
        with open(prompt_path) as f:
            prompts = yaml.safe_load(f)
        # Prompt for generating questions from article (src)
        self.prompt_article = prompts["user_instruction_set"]["qa_gen_article_v0"]
        # Prompt for generating questions from summary (trg/highlights)
    ...
    def generate_questions_from_src(self, example):
        """Generate questions from the source article (src)"""
        rendered_prompt = self.prompt_article.format(
            text=example["src"], 
    ...
    def generate_questions_from_trg(self, example):
        """Generate questions from the target summary (trg)"""
...
# %%
output_path = f"/data/cef/cnndm/cnndm_questions_{judge_model.replace('/', '+++')}"
