#!/usr/bin/env python3
"""
Evaluate ACI (Ambient Clinical Intelligence) note generation using Cross Examination Framework (CEF).

This script evaluates the quality of generated clinical notes by:
1. CEF Evaluation: Generate questions from conversation/note and cross-examine
...
def eval_aci(
    ...
    # Available judges: worker-7 (Llama-3.3-70B), worker-0 (Qwen3-235B), worker-5 (DeepSeek-V3), worker-2 (gpt-oss-120b)
    judge_model: str = "meta-llama/Llama-3.3-70B-Instruct",
    judge_worker: str = "worker-7",
    judge_port: int = 8892,
    ...
    # Prompt settings
    prompt_path: str = "/home/yathagata/cef-translation/src/cef_framework/prompts/note_generation.yaml",
    
    # Data settings
    data_path: str = "/data/cef/aci/aci_qwen3-235b",
    reference_path: str = "/data/cef/aci/aci_original",
    results_base_path: str = "/data/cef/aci/results_cef",
    ...
    """
    Evaluate ACI clinical note generation quality.
    ...
    Args:
        ...
        num_questions_src: Number of questions from conversation (coverage)
        num_questions_trg: Number of questions from note (consistency)
        prompt_path: Path to note generation prompts YAML
        data_path: Path to dataset with generated notes
        reference_path: Path to original dataset with reference notes
        ...
        with_reference: If True, replace src with reference trg (compare generated vs reference note)
    
    Example:
        python eval_aci.py --eval_cef --debug
        python eval_aci.py --eval_cef --with_reference  # Compare against reference notes
    """
    
    # Print arguments
    print("=" * 60)
    print("ACI Clinical Note Generation Evaluation")
    print("=" * 60)
    ...
    # This compares generated note against reference note instead of conversation
    if with_reference:
        ...
        try:
            ...
            logger.info("Successfully replaced src with reference trg (reference clinical notes)")
        except Exception as e:
            logger.error(f"Failed to load reference dataset: {e}")
            logger.warning("Continuing with original src (conversations)")
    ...
    if eval_traditional:
        ...
        for split in selected_splits:
            ...
            try:
                evaluator = ACITraditionalEvaluator(
