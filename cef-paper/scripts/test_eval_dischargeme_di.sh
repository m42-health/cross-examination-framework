#!/bin/bash
# Test script for DischargeME Discharge Instructions Evaluation

echo "Testing DischargeME DI Evaluation Pipeline"
echo "============================================"
echo ""

# Set paths
PREDICTIONS_PATH="/data/cef/dischargeme/dischargeme/DeepSeek-V3.1/predictions/DeepSeek-V3.1_raw_di_responses_tmp.jsonl"
REFERENCE_PATH="/data/cef/dischargeme/gt/discharge-me/1.3"
RESULTS_PATH="/data/cef/dischargeme/results_di_cef"

# Change to evaluation directory
cd /home/yathagata/cef-translation/src/evaluation

echo "Test 1: Traditional Metrics Only (Debug Mode)"
echo "----------------------------------------------"
python eval_dischargeme_di.py \
  --predictions_path="${PREDICTIONS_PATH}" \
  --reference_path="${REFERENCE_PATH}" \
  --results_base_path="${RESULTS_PATH}" \
  --eval_traditional=True \
  --eval_cef=False \
  --debug=True

echo ""
echo "Test completed! Check results in ${RESULTS_PATH}/debug"
echo ""
echo "To run CEF evaluation, use:"
echo "python eval_dischargeme_di.py \\"
echo "  --predictions_path=\"${PREDICTIONS_PATH}\" \\"
echo "  --worker=worker-10 \\"
echo "  --judge_worker=worker-13 \\"
echo "  --judge_port=8000 \\"
echo "  --judge_model=\"medicllama\" \\"
echo "  --eval_cef=True \\"
echo "  --eval_traditional=True \\"
echo "  --num_questions_to_generate=10 \\"
echo "  --debug=True"

