...
#     Resource Request       #
##############################
#SBATCH --job-name=eval_traditional_all
#SBATCH --nodes=1
...
RESULTS_PATH="/data/cef/dischargeme/results_traditional"

cd /home/yathagata/cef-translation/src/evaluation

echo "========================================="
echo "Starting Traditional metrics evaluation for all models"
...
for MODEL_NAME in "${MODELS[@]}"; do
    echo "========================================="
    echo "Traditional Metrics: ${MODEL_NAME}"
    echo "Time: $(date)"
    echo "========================================="
    
    # Check if model is in JSON_MODELS array
    if [[ " ${JSON_MODELS[*]} " =~ " ${MODEL_NAME} " ]]; then
        PREDICTIONS_PATH="${BASE_DIR}/${MODEL_NAME}/predictions/${MODEL_NAME}_raw_bhc_responses.json"
    else
        PREDICTIONS_PATH="${BASE_DIR}/${MODEL_NAME}/predictions/${MODEL_NAME}_raw_bhc_responses_tmp.jsonl"
    fi
