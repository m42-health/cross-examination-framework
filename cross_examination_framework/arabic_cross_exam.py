...
async def evaluate_arabic(
    ...
    """
    Evaluates Arabic language factuality.
    """
    logger.info("Starting Arabic Cross-Examination for factuality evaluation...")
    ...
    try:
        ...
        logger.info("Step 1: Generating QA from Original Document (Arabic)...")
        qa_doc = await generate_qa_from_text(
            ...
            qa_instruction_key="qa_gen_document_arabic"
        )
        
        # Step 2: Generate QA from Summary
        logger.info("Step 2: Generating QA from Summary (Arabic)...")
        qa_sum = await generate_qa_from_text(
            ...
            llm_api=llm_api,
            prompt_catalogue_path=prompt_filepath,
            qa_instruction_key="qa_gen_summary_arabic"
        )
        ...
        doc_answers = await cross_examine(
            ...
            llm_api=llm_api,
            prompt_catalogue_path=prompt_filepath,
            # We assume answers should naturally be in English via standard instructions or LLM translates
            # If we need Arabic answers, we would specify 'answer_instruction_key' here too.
        )
        ...
        logger.info("Step 4: Calculating Arabic Factuality Scores...")
        mock_row = {
        ...
        return {
            "scores": scores,
            "details": {
                "arabic_qa_from_doc": qa_doc,
                "arabic_qa_from_sum": qa_sum,
        ...
        logger.error(f"Error during evaluate_arabic: {e}", exc_info=True)
        return {"error": str(e)}
